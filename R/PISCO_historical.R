...
setwd("D:/Paper_Climate/Paper/Data/climate_/v2_PISCO_RAIN4PE/PISCO/RAW")
rm(list=ls())
...
#                                BC
################################################################################
setwd("D:/Paper_Climate/Paper/Data/climate_/v2_PISCO_RAIN4PE/PISCO/BC/")
...
North <- read.csv("PISCO.csv" ,head = T,check.names = F,stringsAsFactors = F)
...
mediana_WRF3<- median(PT1$Precip, na.rm = TRUE)
PT2$RCM <- c("CSIRO-QCCCE-Mk3")
mediana_RCA4<- median(PT2$Precip, na.rm = TRUE)
PT9$RCM <- c("CCCma-CanESM2-SAM20")
mediana_ETA<- median(PT9$Precip, na.rm = TRUE)

LS1$RCM <- c("CCCma-CanESM2")   #modelo WRF3
...
NUtahMIROC5_RCA4_v3DM$RCM <- c("MOHC-HadGEM2-ES")
...
DM_raw <- rbind(NUtahCanESM2_WRFDM, NUtahCSIRO_Mk3DM,NUtahEC_EARTH_RCA4_v3DM,
                ...
                HadGEM2_RCA4_v3DM,NUtahESM2M_RCA4_v3DM, NUtahRAIN4PE_PIS)
...
#write.csv(Combined_precip , "D:/Paper_Climate/prec_inv.csv", row.names = FALSE)
# Figure
...
boxplot_fig <- ggplot(all_precip, aes(x = run, y = Precip, fill = RCM)) +
  ...
  geom_hline(yintercept = 5.94623, linetype = "dashed", color = "blue") # MEDIANA VERANO 3.998 PISCO
...
# Supongamos que tienes un conjunto de datos llamado "verano_data" con la columna "Precip_verano"
# Combina los datos de invierno y verano en un único conjunto de datos
combined_data <- rbind(
  transform(all_precip, season = "Invierno"),
  transform(all_precip, season = "Verano")
)

# Actualiza el código del gráfico de cajas para incluir ambos conjuntos de datos
boxplot_fig <- ggplot(combined_data, aes(x = run, y = Precip, fill = RCM)) +
  geom_boxplot() + 
  scale_fill_manual(values = c('red', "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", 
                               "#D55E00", "#5acadb", "#83db5a", "#CC79A7", "black")) +
  labs(y = "Proyección de precipitación diaria promedio en invierno (mm), Dec - March", x = " ") +
  xlim(1981, 2004) + 
  theme_bw() +
  facet_wrap(~ BC, ncol = 2, nrow = 3) + 
  geom_hline(yintercept = 5.94623, linetype = "dashed", color = "blue") +  # MEDIANA VERANO 3.998 PISCO
...
boxplot_fig <- ggplot(all_precip, aes(x = run, y = Precip, fill = RCM)) +
  ...
  #geom_boxplot(width = 0.8)+
  facet_wrap(~ BC, ncol=2, nrow = 3)+ 
  geom_hline(yintercept = 3.998 , linetype = "dashed", color = "blue") # MEDIANA VERANO 3.998 PISCO
boxplot_fig <- boxplot_fig +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE, title.position = "bottom", title.hjust = 0.5,title = " ")) +
  theme(
    legend.position = "bottom",  # Coloca la leyenda en la parte inferior
...
#################################################################################
combined_data <- rbind(
  transform(Combined_precip_invierno, season = "DJF"),
...
boxplot_fig <- ggplot(combined_data, aes(x = run, y = Precip, fill = RCM, group = interaction(RCM, season))) +
  ...
  scale_fill_manual(values = c('red', "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", 
                               "#D55E00", "#5acadb", "#83db5a", "#CC79A7", "black")) +
  labs(y = "Proyección de precipitación diaria promedio (mm)", x = "Año") +
  #xlim(1981, 2004) + 
  theme_bw() +
  facet_wrap(~ BC, ncol = 2, nrow = 3) + 
...
boxplot_fig <- ggplot(combined_data, aes(x = run, y = Precip, fill = RCM, group = interaction(RCM, season))) +
  ...
  theme_bw() +
  facet_wrap(~ BC, ncol = 2, nrow = 3) + 
  geom_hline(yintercept = 5.94623, linetype = "dotdash", color = "blue") +  
  geom_hline(yintercept = 3.998972, linetype = "dotdash", color = "red") +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE, title.position = "bottom", title.hjust = 0.5, title = " ")) +
  theme(
    legend.position = "bottom",
    legend.direction = "horizontal",
...
boxplot_fig <- ggplot(combined_data, aes(x = run, y = Precip, fill = RCM, group = interaction(RCM, season))) +
  ...
  annotate("rect", xmin = 1981, xmax = 1992.5, ymin = 0, ymax = 11.7,
           alpha = .01, fill = "cyan") +
  annotate("text", x = 1987, y = 11, label = "DJF", size = 4, color = "orange", family = "serif", fontface = "bold") +
  
  annotate("rect", xmin = 1992.5, xmax = 2004, ymin = 0, ymax = 11.7,
           alpha = .01, fill = "brown") +
  annotate("text", x = 1998, y = 11, label = "JJA", size = 4, color = "blue", family = "serif", fontface = "bold")
...
# Guardar la figura con mayor resolución y tamaño
ggsave("D:/A/boxplot_fig3.png", plot = boxplot_fig, dpi = 600)
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  scale_fill_manual(values = c('red', "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", 
                               "#D55E00", "#5acadb", "#83db5a", "#CC79A7", "black",
                               'red', "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", 
                               "#D55E00", "#5acadb", "#83db5a", "#CC79A7", "black"),
                    name = "RCM & Season") +
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  scale_fill_manual(values = c(
    rep(c('red', '#E69F00', '#56B4E9', '#009E73', '#F0E442', '#0072B2', '#D55E00', '#5acadb', '#83db5a', '#CC79A7', 'black'), 2)
  ),
  name = "RCM & Season") +
  labs(y = "Proyección de precipitación diaria promedio (mm)", x = "BC") +
  theme_bw() +
...
palette4 <- brewer.pal(4, "Purples")  # Puedes elegir otra paleta aquí
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  scale_fill_manual(values = c(
    ...
    'red', '#E69F00', 'blue', 'green', '#F0E442', '#0072B2', '#D55E00', '#5acadb', '#83db5a', '#CC79A7', 'darkgray'
  ),
  name = "RCM & Season") +
  labs(y = "Proyección de precipitación diaria promedio (mm)", x = "BC") +
  theme_bw() +
  geom_hline(yintercept = 5.94623, linetype = "twodash", color = "blue") +
  geom_hline(yintercept = 3.998972, linetype = "dotdash", color = "red") +
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  geom_hline(yintercept = 5.94623, linetype = "twodash", color = "blue", size = 1) +
  geom_hline(yintercept = 3.998972, linetype = "dotdash", color = "red", size = 1) +
  scale_fill_manual(values = combined_palette, name = "RCM & Season") +
  labs(
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  theme(
    ...
    axis.text = element_text(color = "black"),
    axis.ticks = element_line(color = "black"),
    plot.width = unit(10, "in"),    # Ajusta el ancho a 10 pulgadas
    plot.height = unit(8, "in"),
  ) +
  ...
  geom_segment(aes(x = 0.85, y = 11, xend = 0.85, yend = 8.5),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "black", size = 1.3) +
  annotate("text", x = 1.1, y = 10, label = "Before Bias", size = 3.5, color = "black", family = "serif")+
  geom_segment(aes(x = 1.88, y = 11, xend = 1.88, yend = 8.5),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "black", size = 1.3) +
  annotate("text", x = 2.1, y = 10, label = "After Bias", size = 3.5, color = "black", family = "serif")

# Muestra el gráfico
print(boxplot_fig)
ggsave("D:/A/boxplot_fig.png", plot = boxplot_fig, width = 6, height = 4, units = "in", dpi = 300)
...
boxplot_fig <- ggplot(combined_data, aes(x = factor(BC), y = Precip, fill = interaction(RCM, season))) +
  ...
  geom_hline(yintercept = 5.94623, linetype = "twodash", color = "blue", size = 0.7) +
  geom_hline(yintercept = 3.998972, linetype = "dotdash", color = "red", size = 0.7) +
  scale_fill_manual(values = combined_palette, name = "RCM & Season") +
  labs(
    y = "Projection for Avg. Daily Precipitation (mm)",
    x = "",
  ...
  geom_segment(aes(x = 0.85, y = 11, xend = 0.85, yend = 8.5),
               ...
               color = "black", size = 1.3) +
  annotate("text", x = 1.3, y = 10, label = "Before Bias", size = 3.5, color = "black", family = "serif") +
  geom_segment(aes(x = 1.88, y = 11, xend = 1.88, yend = 8.5),
               arrow = arrow(length = unit(0.15, "cm")),
               color = "black", size = 1.3) +
  annotate("text", x = 2.3, y = 10, label = "After Bias", size = 3.5, color = "black", family = "serif")

boxplot_fig <- boxplot_fig +
  theme(panel.background = element_rect(fill = "white"))

# Guardar en buena calidad
ggsave("boxplot_fig.png", plot = boxplot_fig, width = 8, height = 6, units = "in", dpi = 300, type = "cairo-png", compression = "best")
